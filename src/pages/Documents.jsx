            ...
            {documents.map((doc) => (
              <div key={doc.id} className="form-card" style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', marginTop: 'auto' }}>
                  <Link to="/document-genere" style={{ flex: 1, textAlign: 'center', padding: '12px', backgroundColor: '#F8F9FA', color: '#1A1A1A', borderRadius: '100px', fontSize: '13px', fontWeight: '700', textDecoration: 'none' }}>
                    Consulter
                  </Link>
                  {doc.pdf_url ? (
                    <a
